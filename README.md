...
│   ├── siteplan.png        # 부지 계획도 (933KB, 현재 미사용)
...
│   ├── 59A.png             # 59A형 평면도
│   ├── 59T.png             # 59T형 평면도
│   └── amenities/          # index.html #siteinfo에서 사용하는 시설 사진 (영문 파일명)
└── README.md               # 이 파일
```
...
- 소셜 링크: 네이버 카페(`img/cafe-logo.png`, 배경 제거된 로고) / 카카오톡
...
### 단지안내 (`#siteinfo`)
- 단지 배치도(`img/amenities/site-plan.png`) 위에 시설 위치를 % 좌표로 표시한 hotspot(점) 오버레이 (평형 정보 바로 다음, 주요 일정 이전)
- 배치도 좌측 빈 공간(숲 텍스처)에는 `단지 투시도`/`단지 조감도`(원래 `#gallery`에 있던 이미지) 썸네일 hotspot도 배치
...
- 관련 JS는 `index.html`의 `// ===== SITE INFO (단지안내) =====` 블록 (`amenities`/`galleryPhotos` 배열, `siteplanWrap`/`amenityGrid`/`photoGrid`/`amenityModal` 등)
...
- 위 두 이미지는 `#about` 배경(투시도) 등 다른 곳에서도 쓰이므로 `img/` 루트에 그대로 두고 경로만 참조함
- `img/siteplan.png`(구 갤러리 배치도)는 `#siteinfo`의 고해상도 배치도와 중복이라 갤러리에서 제외함; 파일 자체는 미사용 상태로 `img/`에 남아있음
...
- hotspot 좌표 추가/조정 시: `amenities`/`galleryPhotos` 배열에서 해당 항목의 `top`/`left`(배치도 이미지 기준 %) 값을 수정
